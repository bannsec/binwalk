...
class Modules(object):
    ...
    def list(self, attribute="run"):
        ...
        for file_name in os.listdir(user_modules):
            ...
            try:
                _spec = importlib.util.spec_from_file_location(module_name, os.path.join(user_modules, file_name))
                user_module = importlib.util.module_from_spec(_spec)
                _spec.loader.exec_module(user_module)
            except KeyboardInterrupt as e:
                raise e
            except Exception as e:
